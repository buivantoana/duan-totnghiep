    ...
    let handleSaveOrder = async () => {
        if (!dataTypeShip.id) {
            ...
            dataCart.map((item, index) => {
                let object = {};
                object.productId = item.product.id
                object.quantity = item.quantity
                object.realPrice = item.product.discountPrice
                object.color = item.productdetailcolor
                object.image = item.product.image
                object.size = item.productdetailsizeId
                result.push(object)
            })
