...
const getAllProductUser = (data) => {
    return axios.get(`/api/get-all-product-user?limit=${data.limit}&offset=${data.offset}&sortPrice=${data.sortPrice}&sortName=${data.sortName}&categoryId=${data.categoryId}&brandId=${data.brandId}&keyword=${data.keyword}&sizeId=${data.sizeId}&colorId=${data.colorId}`)
...
const searchOrder = (value) => {
    return axios.get(`/api/search-order?value=${value}`)
}
