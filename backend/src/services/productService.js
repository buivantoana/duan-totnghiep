...
let createNewProduct = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.categoryId || !data.brandId || !data.image) {
                resolve({
                    errCode: 1,
    ...
    return new Promise(async (resolve, reject) => {
        try {
            let objectFilter = {
                ...
                distinct: true,  // Important to avoid duplicates
                raw: false, // Ensure raw: false for proper association loading
                nest: true
            }
